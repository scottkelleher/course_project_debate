...
## specify inputs
<<<<<<< HEAD:r markdown combine with janalee after i did major edit6.Rmd
Speaker <- "Hilary_Clinton"  # enter Donald_Trump or Hilary_Clinton
Debate <- "Second_Debate"    # enter First_Debate or Second_Debate or 
                             # Third_Debate
=======
  Speaker <- "Hilary_Clinton"  # enter Donald_Trump or Hilary_Clinton
...
library(googleVis)
library(syuzhet)
>>>>>>> upstream/master:resource_code/revert_back_to_verion_do_not_edit_or_work_from_this_copy_beforebrookesissues.Rmd

# enter up to five phrases to google in gtrends and results
google_these_words <- c("Melania", "email scandal", "bigly",
                        "nasty woman", "benghazi")  
...
# Load the debates 
text_debate1 <- read_html(paste0("http://www.presidency.ucsb.edu/",
                                 "ws/index.php?pid=118971"))
text_debate2 <- read_html(paste0("http://www.presidency.ucsb.edu/",
                                  "ws/index.php?pid=119038"))  
text_debate3 <- read_html(paste0("http://www.presidency.ucsb.edu/",
                                 "ws/index.php?pid=119039"))
...
state_election_results <- read_csv(paste0("https://raw.githubusercontent.com/",
                                      "kshaffer/election2016/master/",
                                      "2016ElectionResultsByState.csv")) %>%
  mutate(state = tolower(state),
         perc_trump = trumpVotes / totalVotes)
...
by_state <-google_breakdown[[5]] %>%
  ...
  dplyr::rename(region = Subregion,
                value = `United.States`) %>% 
  left_join(state_election_results, by = c("region" = "state"))
```


```{r  echo = FALSE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 2.75}
google_state_map <- state_choropleth(by_state) 
searches_vs_votes <- ggplot(by_state, aes(x = value, y = perc_trump)) + 
  geom_point() + 
  xlab("Google searches for 'bigly'") + 
  ylab("Trump votes as percent\nof total votes") + 
  ggtitle("Searches vs. votes\nby state")

library(gridExtra)
grid.arrange(searches_vs_votes, google_state_map, ncol = 2, widths = c(1, 2))
```
...
```{r echo = FALSE, include = FALSE, message = FALSE, fig.width = 8, fig.height = 3}
for (i in (1:3)) {     
  ...
  trump_words <- mutate(trump_words, elim = trump_words_unique,
                        debate = i, date = date_time[i])
  trump_words<- filter(trump_words, trump_words$elim == TRUE)
  trump_words <- select(trump_words, -elim)
  
  ## This is an alternative way you could have done this cleaning: 
  # trump_words <- data_frame(text_t = trump_lines[[1]]) %>%
  #   unnest_tokens(word, text_t) %>%
  ...
  if(i == 1){
    ...
    all_debate_words_clinton <- bind_rows(all_debate_words_clinton,
                                          clinton_words)
  }
...
df <- list(trump = all_debate_words_trump, 
           clinton = all_debate_words_clinton) %>%
  bind_rows(.id = "speaker") %>%
  group_by(speaker, debate) %>%
  dplyr::mutate(n = 1:n(),
                perc_debate = n / n()) %>%
  ungroup() %>%
  mutate(emo_var = get_sentiment(word)) %>%
  group_by(speaker, debate) %>%
  mutate(smooth_emo_var = zoo::rollmean(emo_var, k = bwdw, fill = NA)) %>%
  ungroup() %>%
  mutate(scale_emo_var = 2 * (smooth_emo_var - 
                                min(smooth_emo_var, na.rm = TRUE)) / 
           (max(smooth_emo_var, na.rm = TRUE) - 
              min(smooth_emo_var, na.rm = TRUE)) - 1)
ggplot(df, aes(x = perc_debate, y = scale_emo_var, color = speaker)) + 
  geom_line() + 
  # geom_smooth(se = FALSE, alpha = 0.8, span = 0.05) + 

...
debate_dates <- data.frame(number = c(1, 2, 3),
                           date = ymd(c("2016-09-27", "2016-10-09",
                                        "2016-10-29")))

google_results$trend %>%
  mutate(start = ymd(start)) %>%
  ggplot(aes(x = start, y = hits, color = keyword)) + 
  geom_line() + 
  theme_classic() + 
  xlab("Date") + 
...
db1 <- ymd_hm("2016-09-27 02:00", tz = "UTC")
db2 <- ymd_hm("2016-10-09 02:00", tz = "UTC")
db3 <- ymd_hm("2016-10-29 02:00", tz = "UTC")
...
#to see each plot for each debate, uncomment the rest of the chunk
...
```{r   echo = FALSE, message = FALSE, eval= FALSE, include = FALSE, fig.width = 6, fig.height=3}
# not really using this one for anything but do not erase 
clinton_x <- rename(clinton_x, c(`clinton_list_getting$x` = "score"))
clinton_y <- rename(clinton_y, c(`clinton_list_getting$z` = "score"))
...
ggplot(df_t, aes(y= trump_plot_y, x = trump_plot_x, colour = factor(debate)))+
  scale_color_discrete(name = "Debate")+
  geom_line()+
...
```{r echo=FALSE, message = FALSE, error= FALSE, warning = FALSE, eval = FALSE, fig.width = 6, fig.height=3}
...
# lines(trump_list$x[bov_sample], trump_list$z[bov_sample], col="red")
# 
# ##feel fre to unomment some code to see more plots pop up
# 
# plot(
#   s_v_sentiment_c, 
#   s_v_sentiment_t,
...
```{r echo = FALSE, message = FALSE, warning = FALSE, fig.width = 6, fig.height=3}
 trump_emo_plot_this_stuff <- rename(trump_emo, c(`colSums(prop.table(nrc_data_t[, 1:8]))` = "emo_val",
`rownames(trump_emo)` = "what_emotion"))
ggplot(trump_emo_plot_this_stuff, aes(y = emo_val, x = what_emotion, fill = factor(debate)))+
geom_bar(position="dodge", stat="identity") +
  scale_y_continuous("Percent of Total", breaks = c(0, 0.05, 0.10, 0.15, 0.20, 0.25), 
                     labels = c("0", "5%", "10%", "15%", "20%", "25%")) +
  ggtitle("Trump Sentiment") +
scale_fill_discrete(name = "Debate")+
theme(plot.title = element_text(hjust = .5))
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.width = 6, fig.height=3}
 clinton_emo_plot_this_stuff <- rename(clinton_emo, c(`colSums(prop.table(nrc_data_c[, 1:8]))` = "emo_val", `rownames(clinton_emo)` = "what_emotion"))
ggplot(clinton_emo_plot_this_stuff, aes(y = emo_val, x = what_emotion, fill = factor(debate)))+
  geom_bar(position="dodge", stat="identity") +
  scale_y_continuous("Percent of Total", breaks = c(0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30), 
